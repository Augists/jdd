...
public class TestZDDCSP {
	...
	@Test public void testAll() {
		...
		assertEquals("TOS restored (1)", csp.debug_work_stack_size(), 0 );


		// test P / Q
		int answer_division = 0;
		assertEquals("P / Q", answer_division, csp.div(p,q) );
		assertEquals("TOS restored (2)", csp.debug_work_stack_size(), 0);


		// test P % Q
		int answer_quotient = csp.cubes_union("00011 00111 01110");
		assertEquals("P % Q", answer_quotient, csp.mod(p,q));
		assertEquals("TOS restored (3)", csp.debug_work_stack_size(), 0);
		...
		assertEquals("TOS restored (4)", csp.debug_work_stack_size(), 0);
		...
		assertEquals("TOS restored (5)", csp.debug_work_stack_size(), 0);
	}
}
